...
for i in range(len(cont1)):
    ...
    if (op1[i] == "Moved") and (obj1[i] == "file:"):
        file = [x for x in cont1[i][-3].split('/') if x not in dir1.split('/')]
        if (file[-1] != filenames1[i][-1]):
            if (filenames1[i] in modfiles):
...
for i in range(len(cont2)):
    ...
    if (op2[i] == "Moved") and (obj2[i] == "file:"):
        file = [x for x in cont2[i][-3].split('/') if x not in dir2.split('/')]
        if (file[-1] != filenames2[i][-1]):
            if (filenames2[i] in modfiles):
...
for i in range(len(cont1)):
    if op1[i] == "Deleted" and obj1[i] == "file:":
        ...
        file = [x for x in cont1[i][-3].split('/') if x not in dir1.split('/')]
        if paths1[i].split('/')[-1] == file[-1]:
            if os.path.isfile(join(dir2, file)):
...
for i in range(len(cont2)):
    if op2[i] == "Deleted" and obj2[i] == "file:":
        ...
        file = [x for x in cont2[i][-3].split('/') if x not in dir2.split('/')]
        if (filenames2[-1] == file[-1]):
            if os.path.isfile(join(dir1, file)):
...
for i in range(len(modfiles)):
    if part[i][2] == dir1:
        if os.path.isfile(join(dir1, modfiles[i])):
            cmd = f'rsync -a {join(dir1, modfiles[i])} {join(dir2, modfiles[i])}'
            call(cmd.split())
    else:
        if os.path.isfile(join(dir2, modfiles[i])):
